...
function KakaoLogin() {
  const searchParams = useSearchParams();
    ...
    async function checkValidAccess(){
        try{
            const {message, error} = await get(`/soon/isValid-kakao-login-register?userId=${userId}`)
            if(error){
                alert(error);
                return false;
            }
    ...
    return <Stack gap="12px" alignItems="center" justifyContent="center" height="100vh">
