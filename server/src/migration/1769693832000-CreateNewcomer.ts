...
export class CreateNewcomer1769693832000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. NewcomerManager 테이블 생성
    await queryRunner.query(`
      CREATE TABLE \`newcomer_manager\` (
        \`id\` varchar(36) NOT NULL,
        \`userId\` varchar(36) NULL,
        PRIMARY KEY (\`id\`),
        UNIQUE INDEX \`UQ_newcomer_manager_user\` (\`userId\`),
        CONSTRAINT \`FK_newcomer_manager_user\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE SET NULL ON UPDATE NO ACTION
